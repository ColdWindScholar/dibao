...
#3
  ...
  $github
   ...
   if [ $os = 'Android' ];then
   if [ ! -d /data/底包直刷包制作工具/product/product ];then
   echo "\033[33m注意！/data/底包直刷包制作工具的product文件夹为空！\033[0m"
   fi   
   if [ ! -f ~/python/e2fsdroid ];then
  echo "\033[31m$error\033[0m"
   rm -rf ~/python
   ...
   if [ $os = 'GNU/Linux' ];then
   if [ ! -d /root/底包直刷包制作工具/product/product ];then
   echo "\033[33m注意！/root/底包直刷包制作工具的product文件夹为空！\033[0m"
   fi   
   if [ ! -f ~/python/e2fsdroid~ ];then
  echo "\033[31m$error\033[0m"
   rm -rf ~/python
   ...
   if [ $os = 'Android' ];then
   ssize=$(cat /data/底包直刷包制作工具/product/config/product_size.txt)
   fi
   if [ $os = 'GNU/Linux' ];then
   ssize=$(cat /root/底包直刷包制作工具/product/config/product_size.txt)
   fi   
   size=$(expr $ssize / 1024 / 1024 )
   read -p "自定义打包大小(直接按回车使用默认配置文件打包){不用带单位,默认单位:M}: " zdydbdx   
   if [ $zdydbdx ];then
   ...
   sb=$(echo "${size}M")
   read -p "是否打包product.img?[$sb](y/n): " mkesys
   if [ -z $mkesys ];then
   if [ $os = 'Android' ];then   
   echo "\033[32m正在打包product.img……\033[0m"   
   su -c ~/python/mke2fs -L / -t ext4 -b 4096 $bf/工作目录/IMG/product_new.img ${size}M   
   su -c ~/python/e2fsdroid -e -T 0 -S /data/底包直刷包制作工具/product/config/product_file_contexts -C /data/底包直刷包制作工具/product/config/product_fs_config  -a /product -f /data/底包直刷包制作工具/product/rroduct $bf/工作目录/IMG/product_new.img
   fi
   if [ $os = 'GNU/Linux' ];then
   echo "\033[32m正在打包product.img……\033[0m"   
   ~/python/mke2fs~ -L / -t ext4 -b 4096 $bf/工作目录/IMG/product_new.img ${size}M   
   ~/python/e2fsdroid~ -e -T 0 -S /root/底包直刷包制作工具/product/config/product_file_contexts -C /root/底包直刷包制作工具/product/config/product_fs_config  -a /product -f /root/底包直刷包制作工具/product/product $bf/工作目录/IMG/product_new.img
   fi      
   fi         
   if [ $mkesys = 'y' ];then  
   if [ $os = 'Android' ];then   
   echo "\033[32m正在打包product.img……\033[0m"   
   su -c ~/python/mke2fs -L / -t ext4 -b 4096 $bf/工作目录/IMG/product_new.img ${size}M   
   su -c ~/python/e2fsdroid -e -T 0 -S /data/底包直刷包制作工具/product/config/product_file_contexts -C /data/底包直刷包制作工具/product/config/product_fs_config  -a /product -f /data/底包直刷包制作工具/product/product $bf/工作目录/IMG/product_new.img
   fi
   if [ $os = 'GNU/Linux' ];then
   echo "\033[32m正在打包product.img……\033[0m"   
   ~/python/mke2fs~ -L / -t ext4 -b 4096 $bf/工作目录/IMG/product_new.img ${size}M   
