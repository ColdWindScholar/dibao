...
#1
   ...
   read -p "请将raw格式的system.img放到工作目录/IMG里，按任意键继续" me
   ...
   echo "\\033[31m修改失败！请确认system.img是否已放到工作目录/IMG里。\033[0m"
   read -p "按任意键返回" make
   cd $af
   clear
   sh $0   
   ...
   su -c losetup /dev/block/loop5 $bf/工作目录/IMG/system.img
   su -c mount /dev/block/loop5 /工作目录/system
   fi
   if [ $os = 'GNU/Linux' ];then   
   mount -o remount -rw /
   mkdir /工作目录 /工作目录/system 
   losetup /dev/loop5 $bf/工作目录/IMG/system.img
...
#2
   ...
   read -p "请将raw格式的vendor.img放到工作目录/IMG里，按任意键继续:" me   
   echo "\033[33m(linux用户忽略)请在面具》设置》挂载命名空间模式选择全局命名空间，否则挂载了看不见里面的文件(如果是第一次设置请关闭一次Termux再打开重新执行一下脚本)。\033[0m"
   read -p "按任意键继续" xg
   if [ ! -f $bf/工作目录/IMG/vendor.img ];then
   echo "\\033[31m修改失败！请确认vendor.img是否已放到工作目录/IMG里。\033[0m"
   read -p "按任意键返回" make
   cd $af
   clear
   sh $0   
   ...
   su -c losetup /dev/block/loop4 $bf/工作目录/IMG/vendor.img
   su -c mount /dev/block/loop4 /工作目录/vendor
   fi
   if [ $os = 'GNU/Linux' ];then   
   mount -o remount -rw /
   mkdir /工作目录 /工作目录/vendor   
   losetup /dev/loop4 $bf/工作目录/IMG/vendor.img
...
if [ $ab = '3' ];then
   ...
   if [ -z $tz ];then
   read -p "请将raw格式的system.img放到工作目录/IMG文件夹里,按任意键继续" make   
   if [ ! -f $bf/工作目录/IMG/system.img ];then
   echo "\033[31m未检测到system.img，请确认system.img是否已放到工作目录/IMG里。\033[0m"
   read -p "按任意键返回" make   
   cd $af
   clear
   sh $0     
   ...
   e2fsck -fy $bf/工作目录/IMG/system.img
   resize2fs -f $bf/工作目录/IMG/system.img ${vt}m
   if [ $? = '0' ];then
...
if [ $ab = '4' ];then
   ...
   if [ -z $tz ];then
   read -p "请将raw格式的vendor.img放到工作目录/IMG文件夹里,按任意键继续" make   
   if [ ! -f $bf/工作目录/IMG/vendor.img ];then
   echo "\033[31m未检测到vendor.img，请确认vendor.img是否已放到工作目录/IMG里。\033[0m"
   read -p "按任意键返回" make   
   cd $af
   clear
   sh $0     
   ...
   e2fsck -fy $bf/工作目录/IMG/vendor.img
   resize2fs -f $bf/工作目录/IMG/vendor.img ${vt}m
   if [ $? = '0' ];then
   echo "\033[32m已调整，请检查img文件的大小是否和输入的数值一致。\033[0m"
   else
   echo "\033[31m调整失败，请检查输入数值是否大于镜像文件里的真实文件大小。\033[0m"   
